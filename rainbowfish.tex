...
% Author macros::begin %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\title{\rainbowfish: A Succinct Colored de Bruijn Graph Representation}
%\titlerunning{\rainbowfish} %optional, in case that the title is too long; the running title should fit into the top page column
...
%
%In this paper, we introduce \rainbowfish, a succinct representation of the color
...
%distribution of these color sets. The \rainbowfish representation is compressed based
...
%\rainbowfish achieves up to a $20\times$ improvement in space over \vari. \rainbowfish is
...
\dbg edge a distinct rank in the range $[0,N)$. So, \vari represents the color
  ...
  In this paper, we introduce a succinct representation, called \rainbowfish, of the
...
%In \rainbowfish, we construct the color class representation from a color list from
...
%exact value of \kmer that has $rank=i$. Both \vari and \rainbowfish consider \kmers
%to be in the same order as returned by rank operation on \boss representation
%of \dbg. \rainbowfish uses rank-and-select operation to map a \kmer to its color
...
%one \rainbowfish leverages the skewness property of distribution of \kmers across
...
class table, \rainbowfish maintains two bit vectors, a \defn{boundary bit vector
...
\Cref{fig:cdbg-rep} shows how the color classes are represented in \rainbowfish. To
...
However, as explained in \Cref{design}, in \rainbowfish we assign (optimal) variable-length
...
%bit vector in \rainbowfish depends on the entropy of the color class distribution.
...
Now, we show that the space needed by \rainbowfish to store the variable-length
...
\subparagraph{Storing bit vectors.} In \rainbowfish, we use bit vector implementations
...
As explained in \Cref{rainbowfish-space-bound}, the $2$-pass algorithm minimizes the
...
%\kmers in the space required by \rainbowfish we ran it \kmers with $k=63$ from
%\ecoli 1000.
...
%performance of \rainbowfish, we design three different experiments on \ecoli data
...
%practical example to evaluate time and space of \rainbowfish compared to
...
\Cref{tab:time} shows the time taken by \rainbowfish and \vari to construct the color
...
\paragraph*{~Space}\Cref{tab:space} shows the space usage of \rainbowfish and \vari
...
%of colors. As shown in table \cref{tb:space} \rainbowfish improves space when
...
%\plant dataset although it has just four colors which implies that \rainbowfish not
...
%sets in \rainbowfish can still save a lot of space when we have a considerable
%amount of \kmers even with the overload of saving equivalence table.

\paragraph*{~Bubble calling} \Cref{tab:time} shows the time taken by \rainbowfish and \vari to perform the bubble
...
%
%\fatemeh{Since
%\rainbowfish is dependent on color matrix as input, the building process } 
%
...
%process. Yet, we can build all the data structures in \rainbowfish by just going over
...
%\fatemeh{One of these representations is compacted \dbg in which
  ...
  %improve the space \rainbowfish takes by decreasing number of \kmers we have to
